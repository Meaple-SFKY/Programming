...
int pDepth(pTree& T, elementType x)
{
	int w = 0;
	int i = 0;
	elementType y;
...
void preOrderDepth(pTree& T, int v)
{
	int w;
	cout << "(" << T.node[v].data << ", " << pDepth(T, T.node[v].data) << ")" << "  ";
	w = firstChild(T, v);
	while (w != -1)
	{
		preOrderDepth(T, w);
...
void create(csNode*& T, pTree& T1, int v)
{
	...
	T->firstChild = NULL;
	T->nextSibling = NULL;
	T->depth = pDepth(T1, T1.node[v].data);
